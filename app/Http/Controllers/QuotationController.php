...

{
...

    {
        ...
        if($role->hasPermissionTo('quotes-index')){
           ...
           $quotation_list = Quotation::with(['warehouse'])->with('quotationemail')->with('quotationitem')->orderBy('id','desc')->where('is_active','1')->get();
     ...
     //  dd( $quotation_list);
        ...
        // foreach($quotation_list as $qt){

        // $qemail = $qt->quotationemail;

        //     foreach($qemail as $qe){

        //         $supplier = Supplier::find($qe->supplier_id);
...
// dd(array_unique($supplier_name));

            return view('quotation.index', compact('quotation_list','all_permission'));
    ...
    public function supplierResponseList(Request $request)

    {

       

        $qitems = QuotationItem::with('product')->where('quotation_id',$request->id)->get();



        $quotation_response = QuotationResponse::with('product')->where('quotation_id',$request->id)->get();
        ...
        $qritems = QuotationResponseItem::with(['product','qrf'])->whereIn("quotation_response_id",$ids);
        ...
        if($request->product){
...

            {
               ...
               $qritems = $qritems->where("product_id",$request->product);
        ...
        $qritems = $qritems->orderBy("price","asc")->get();
        ...
        // dd($qritems);
         ...
         return view('quotation.supplier-response-list',compact('quotation_response','qitems','qritems'));

    }

   

    public function supplierResponse($id)

    {
        Session::put('supplier', $id);
       $supplier = QuotationEmail::with('supplier')->where('ref_no',$id)->first();
        ...
        return view('quotation.supplier-response',compact('quotation_response','supplier','brandData'));

    }

    public function adminToSupplierReponse(Request $request,$slug)
    {
        foreach ($request->price as $k=>$v) {
            ...
            for($i=0;$i<count($v);$i++){
                ...
                if($v[$i]!=null && $request->brand[$k][$i]!=null){
                    print_r($v);
                    $brands[] = array("brand"=>$request->brand[$k][$i],"price"=>$v[$i],"new_price"=>$request->new_price[$k][$i]);

                }

            }
            dd($request,$brands);
            $quotation_response = QuotationResponse::where('quotation_id',$request->quotation_id)->get();
...

    {
        ...
        if($role->hasPermissionTo('quotes-add')){
            ...
            return view('quotation.create', compact('lims_biller_list','lims_product_list', 'lims_warehouse_list', 'lims_customer_list', 'lims_supplier_list', 'lims_tax_list'));
...

    {
        ...
        $this->validate($request, [
            ...
            // 'qty' => 'required_if:product_code_name,==,required'
            ...
            'supplier_id.required' => 'Kindly Select Supplier ',

            'warehouse_id.required' => 'Kindly Select Warehouse ',

            'product_code_name.required' => 'Kindly Select Product',

            'qty.required' => 'Kindly Enter Product Quantity',
        ...
        foreach($request->qty as $key=>$v){
            ...
            $quotation_item->product_id = $key;

            $quotation_item->qty = $v;
        ...
        foreach($request->supplier_id as $spl_id){



        $quotation_email = new QuotationEmail();



        $quotation_email->quotation_id = $quotation->id;

        $quotation_email->supplier_id = $spl_id;



        $ref_no = time().uniqid().$spl_id;

        //print_r($ref_no);

        $quotation_email->ref_no = $ref_no;

        $quotation_email->save();

            //dd($quotation_email->supplier_id);

        $supplier_email_id = Supplier::find( $quotation_email->supplier_id);



        $share_link = 'http://kheltoh.tech/supplier-view-quotation-list/'. $ref_no;



        $data = ['message' => 'This is a test!','email_id' => $supplier_email_id->email,'share_id' => $share_link,'ref_no' =>$ref_no ];



        Mail::to($supplier_email_id->email)->send(new SupplierQuatEmail($data));
      ...
      //  dd('hare');
       ...
       return redirect('quotations')->with('message', 'Quotation create successfully');
       ...
       //  return redirect('quotations-email-send')->with('message', $message);
    ...
    public function quotationProductSearchList(Request $request)

    {

       $product_list = Product::where('id',$request->key)->first();
